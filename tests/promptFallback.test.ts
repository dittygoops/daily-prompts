...
import type { DailyPrompts, PromptSource } from "../src/prompts/types";

function stubSource(behavior: () => Promise<DailyPrompts>): PromptSource {
  return { nextPrompts: behavior };
}

const daily = (aText: string, bText: string, theme: string | null = "test theme"): DailyPrompts => ({
  theme,
  prompts: { a: { id: "gen-1-a", text: aText }, b: { id: "gen-1-b", text: bText } },
});

describe("FallbackPromptSource", () => {
  test("returns the primary's prompts when generation succeeds, and never calls the fallback source", async () => {
    const ledger = Ledger.open(":memory:");
    let fallbackCalled = false;
    const primary = stubSource(async () => daily("primary a", "primary b"));
    const fallback = stubSource(async () => {
      fallbackCalled = true;
      return { theme: null, prompts: { a: { id: "p1", text: "fallback prompt" }, b: { id: "p1", text: "fallback prompt" } } };
    });
    const source = new FallbackPromptSource(primary, fallback, { ledger, log: () => {} });
    const result = await source.nextPrompts("2026-07-20");
    expect(result).toEqual(daily("primary a", "primary b"));
  ...
  test("falls back to the fallback source when the primary throws", async () => {
    ...
    const fallback = stubSource(async () => ({ theme: null, prompts: { a: { id: "p1", text: "fallback prompt" }, b: { id: "p1", text: "fallback prompt" } } }));
    const source = new FallbackPromptSource(primary, fallback, { ledger, log: () => {} });
    const result = await source.nextPrompts("2026-07-20");
    expect(result).toEqual({ theme: null, prompts: { a: { id: "p1", text: "fallback prompt" }, b: { id: "p1", text: "fallback prompt" } } });
  });
  ...
  test("falls back on a simulated memory outage error from the primary", async () => {
    ...
    const fallback = stubSource(async () => ({ theme: null, prompts: { a: { id: "p1", text: "fallback prompt" }, b: { id: "p1", text: "fallback prompt" } } }));
    const source = new FallbackPromptSource(primary, fallback, { ledger, log: () => {} });
    await expect(source.nextPrompts("2026-07-20")).resolves.toEqual({ theme: null, prompts: { a: { id: "p1", text: "fallback prompt" }, b: { id: "p1", text: "fallback prompt" } } });
  });
  ...
  test("if the fallback source itself throws, the error propagates (no further fallback)", async () => {
    ...
    const source = new FallbackPromptSource(primary, fallback, { ledger, log: () => {} });
    await expect(source.nextPrompts("2026-07-20")).rejects.toThrow(/fallback also broke/);
  });

  test("never mutates or wraps the primary's successful DailyPrompts", async () => {
    const ledger = Ledger.open(":memory:");
    const exact: DailyPrompts = daily("exact a", "exact b");
    const primary = stubSource(async () => exact);
    const fallback = stubSource(async () => ({ theme: null, prompts: { a: { id: "p1", text: "fallback" }, b: { id: "p1", text: "fallback" } } }));
    const source = new FallbackPromptSource(primary, fallback, { ledger, log: () => {} });
    const result = await source.nextPrompts("2026-07-20");
