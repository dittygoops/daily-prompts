...
- Today's stance is assigned to you below and is not yours to choose. Follow it exactly and echo it back in "stance".
  ...
  - BOTH PEOPLE MUST BE ABLE TO ANSWER IT. They receive the identical prompt, so a question built on one person's experience leaves the other with nothing to say. "How did the psychic reading party go?" is unanswerable by the partner who was not there, and it went out live. When you exploit a thread that belongs to only one of them, widen it so the other has a real way in: ask about the shared category rather than the private instance, or ask something the thread makes vivid that both can speak to from their own side. If a thread cannot be widened that way, pick a different thread.
