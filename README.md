...
Messages and answers transit two third parties: **Photon** (the iMessage delivery service, photon.codes) and **OpenRouter** (LLM API, used to extract per-person memory from each day's answers, and to generate each day's question from that memory). Derived memory (not verbatim messages) lives in the same local SQLite file, `ledger.db`, as conversation history itself, on the machine running the daemon, permissioned `chmod 600`: none of it leaves your Mac except in transit through the two services above. Explore-lane questions are not generated freely; they come from a hand-written, checked-in seed bank (`data/seeds.tsv`). Both participants should know this before the first prompt goes out.
...
What each day's question targets is decided in code, not by the model: the model only writes the sentence. Selection picks a lane, in strict priority. Lane 0, follow-up: an unspent token from a dated event surfaced by extraction, fired if its date falls within a short recent window, bypassing every cooldown below (an appointment outranks hygiene). Lane 1, exploit: an open node with budget remaining, threads first, ordered by richness and staleness. Lane 2, seeded explore: a hand-written seed whose domain and family haven't been used too recently. Which lane wins is governed entirely by windows, not model judgment: a subject cooldown, a domain cooldown, a family cooldown, and a short settling period after a fact is first observed, all vetoes only, never promotions. This started as a model judgment call (`src/prompts/stance.ts`'s exploit/explore split) and the model chose explore on 6 out of 6 live days, so the decision moved into code and then, with this change set, into the fuller lane-and-window system above. The chosen lane and target are recorded in `generation_log` (`lane`, target, `ask_domain`, `ask_family`). A generated prompt that near-duplicates one already sent is rejected and regenerated, using the same deterministic content-word check the eval harness uses. If generation fails for any reason (OpenRouter down, a malformed response, the writer's target mismatch surviving a retry), it falls back to the static 30-question bank (`data/prompts.json`) so the daily message always goes out; every fallback is logged loudly and recorded in the ledger's `generation_log` table alongside every successful generation's full context and reasoning, for later review. Suggested prompt ideas are tracked durably (never silently expire) until the generator actually uses one.
...
The generated-prompt report adds what a single-prompt score cannot see: near-duplication against everything sent before it, reuse of an opening sentence frame, the lane mix (follow-up/exploit/explore), and the fallback rate.
